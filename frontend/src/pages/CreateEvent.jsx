...
const CreateEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const filteredTimeline = formData.timeline.filter(item => item.time.trim() !== '' && item.activity.trim() !== '');
          ...
          <div className="flex items-center gap-4 bg-surface-low p-2 rounded-2xl border border-ghost-border shadow-inner">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black transition-all duration-500 border ${step >= s
            ...
            {step === 1 && (
              ...
              >
                ...
                <div className="space-y-10">
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-on-surface-variant uppercase tracking-[0.3em] ml-1">Event Classification (Category)</label>
                    <div className="relative group flex gap-4">
                      <div className="relative flex-1 group">
                        <LayoutGrid size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-on-surface-variant/40 group-focus-within:text-primary-neon transition-colors pointer-events-none" />
                        <select
                          className="w-full bg-surface-low border border-ghost-border rounded-xl py-5 pl-16 pr-8 text-sm font-bold text-white outline-none focus:border-primary-neon/50 focus:bg-surface-highest transition-all shadow-inner appearance-none cursor-pointer"
                          value={formData.category}
                          onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                          required
                        >
                          {categories.map(cat => (
                            <option key={cat._id} value={cat._id} className="bg-bg-obsidian text-white">{cat.name}</option>
                          ))}
                        </select>
                      </div>
                      {isAdmin && (
                        <button
                          ...
                          className="p-5 bg-primary-neon/10 border border-primary-neon/30 rounded-xl text-primary-neon hover:bg-primary-neon hover:text-bg-obsidian transition-all shadow-glow"
                ...
                <div className="space-y-8">
                  <div className="space-y-4">
                    ...
                    <div
                      ...
                      className={`w-full aspect-video rounded-2xl border-2 border-dashed transition-all cursor-pointer relative overflow-hidden flex flex-col items-center justify-center gap-4 ${formData.imagePreview ? 'border-primary-neon/30' : 'border-ghost-border hover:border-primary-neon/30 bg-surface-low/50'
                        }`}
                    >
